...
- **Plugin:** `src/plugins/plugin-sentinel/`. Actions: **SENTINEL_SUGGEST** (general suggestions + task brief for Claude 4.6), **SENTINEL_CLAWDBOT_GUIDE**, **SENTINEL_SETTINGS_SUGGEST**, **SENTINEL_ONNX_STATUS**, **SENTINEL_ART_GEMS**, **SENTINEL_DOC_IMPROVE**, **SENTINEL_COST_STATUS** (burn rate, breakeven, cost summary from TREASURY + Usage tab).
